...
def ensure_dir(path):
    """Create folders safely. If a file has the same name, replace it with a folder."""
...
def resolve_data_dir():
    ...
    2) /var/data on Render Persistent Disk.
    3) local project folder for testing only.
    """
    candidates = [os.environ.get("DATA_DIR"), "/var/data", os.path.join(BASE_DIR, "data")]
...
# PostgreSQL is the official cloud-save mode. SQLite is only a fallback, saved in DATA_DIR.
app.config["SQLALCHEMY_DATABASE_URI"] = raw_db_url or "sqlite:///" + os.path.join(DATA_DIR, "rufa_gold.db")
...
def init_db():
    ...
    if not admin:
        ...
        admin.password_hash = generate_password_hash("QQ1122qq11223")
...
{% for p in rows %}<tr><td>{% if images.get(p.id) %}<img class="image-thumb" src="{{ url_for('uploaded_file', filename=images.get(p.id)) }}">{% else %}-{% endif %}</td><td>{{ p.code }}</td><td>{{ p.project_name }}</td><td>{{ p.property_type }}</td><td>{{ p.district }}</td><td>{{ p.price|money }}</td><td>{{ status_badge(p.status)|safe }}</td><td class="actions">{% if user.role == 'admin' %}<a class="btn btn-light" href="{{ url_for('property_edit', property_id=p.id) }}">تعديل</a><a class="btn btn-gold" href="{{ url_for('property_sell', property_id=p.id) }}">بيع/سعي</a>{% endif %}</td></tr>{% endfor %}</table></div>
"""
