...
# حفظ دائم: استخدم PostgreSQL عبر DATABASE_URL.
# إذا لم يوجد DATABASE_URL، نحفظ SQLite والملفات داخل DATA_DIR أو /var/data إذا كان موجوداً.
# في Render يجب إضافة Persistent Disk على /var/data أو استخدام PostgreSQL حتى لا تختفي البيانات بعد إعادة النشر.
def pick_data_dir():
    env_dir = os.environ.get("DATA_DIR")
    candidates = [env_dir, "/var/data", BASE_DIR]
...
DB_FILE = os.path.join(DATA_DIR, "rufa_gold.db")
...
CLOUD_SAVE_ENABLED = bool(raw_db_url)
# الحفظ السحابي المعتمد: عند وجود DATABASE_URL يتم حفظ كل البيانات في PostgreSQL مباشرة.
# SQLite يستخدم للتجربة المحلية فقط وليس للاعتماد النهائي على Render.
app.config["SQLALCHEMY_DATABASE_URI"] = raw_db_url or "sqlite:///" + DB_FILE
...
def ensure_sqlite_columns():
    """ترقية بسيطة لقاعدة SQLite القديمة حتى لا يتوقف الحفظ بعد تحديث الكود."""
    ...
    required = {
        ...
        "property": {
            "code": "VARCHAR(30)",
            "project_name": "VARCHAR(200) NOT NULL DEFAULT 'عقار'",
            "property_type": "VARCHAR(80) NOT NULL DEFAULT 'غير محدد'",
            "district": "VARCHAR(120) NOT NULL DEFAULT 'غير محدد'",
        ...
        "property_image": {"property_id": "INTEGER", "filename": "VARCHAR(255) NOT NULL DEFAULT ''", "created_at": "DATETIME"},
        "task": {"title": "VARCHAR(200) NOT NULL DEFAULT ''", "details": "TEXT DEFAULT ''", "assigned_to_id": "INTEGER", "created_by_id": "INTEGER", "status": "VARCHAR(30) DEFAULT 'open'", "note": "TEXT DEFAULT ''", "attachment": "VARCHAR(255)", "created_at": "DATETIME", "completed_at": "DATETIME"},
        "deal": {"property_id": "INTEGER", "commission_amount": "NUMERIC(14, 2) DEFAULT 0", "company_amount": "NUMERIC(14, 2) DEFAULT 0", "sold_at": "DATE", "created_at": "DATETIME"},
        "deal_share": {"deal_id": "INTEGER", "user_id": "INTEGER", "percent": "NUMERIC(7, 2) DEFAULT 0", "amount": "NUMERIC(14, 2) DEFAULT 0"},
        "expense": {"title": "VARCHAR(200) NOT NULL DEFAULT ''", "category": "VARCHAR(80) DEFAULT 'other'", "amount": "NUMERIC(14, 2) DEFAULT 0", "expense_date": "DATE", "created_at": "DATETIME"},
        "finance_entry": {"title": "VARCHAR(200) NOT NULL DEFAULT ''", "category": "VARCHAR(80) DEFAULT 'other'", "entry_type": "VARCHAR(30) DEFAULT 'allocation'", "amount": "NUMERIC(14, 2) DEFAULT 0", "deal_id": "INTEGER", "notes": "TEXT DEFAULT ''", "created_at": "DATETIME"},
    }
...
<main class="main">
<div class="topbar"><div class="userbox"><div class="avatar">👤</div><div><b>{{ user.name }}</b><div class="small">{{ user.role|role_name }}</div></div></div><div class="small">{% if cloud_save %}☁️ الحفظ السحابي مفعل{% else %}⚠️ تجربة محلية - أضف DATABASE_URL للحفظ السحابي{% endif %} · آخر حفظ تلقائي {{ now }}</div></div>
...
"""
    return render_template_string(shell, content=content, title=title, user=user, now=datetime.now().strftime("%Y-%m-%d %H:%M"), active=active, cloud_save=CLOUD_SAVE_ENABLED)
